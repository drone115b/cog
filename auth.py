...
#
# there exists a directory (confdict['COG_AUTHPATH']) which contains two kinds of files:
...
def _make_userpass_filename ( username, confdict ):
    return os.path.join( confdict['COG_AUTHPATH' ], "%s.bin" % username )

def _make_method_permission_filename( confdict ):
    return os.path.join( confdict['COG_AUTHPATH' ], '_methods.json' )
...
class MethodPermissions ( object ):
    def __init__( self, confdict, logger ) :
        ...
        self._deltatime = datetime.timedelta( seconds=confdict['COGSERVER_PERMISSIONS_EXPIRY'] )
